...
flags.DEFINE_list('hidden', [200,200,200,200,'meanmax',-6], 'Number of units in each layer negative:denselayer, positive:ConvGraph layer ')
flags.DEFINE_list('activation_funcs', [tf.nn.relu,tf.nn.relu,tf.nn.relu, tf.nn.relu,None,lambda x: x], 'Activation functions for layers  [tf.nn.relu, lambda x: x]')
flags.DEFINE_list('biases', [False,False,False,False,None,True], 'if apply bias on layers')
flags.DEFINE_list('isdroput_inp', [True,True,True,True,None,False], 'if apply dropout on layers'' input')
flags.DEFINE_list('isdroput_kernel', [True,True,True,True,None,False], 'if apply dropout on layers'' kernel')
flags.DEFINE_list('firstDWS_learnable', [True,True,True,True,None,None], 'if first kernel''s depthwise weights are learnable or not')
flags.DEFINE_list('isdepthwise', [False,False,False,False,None,None], 'if layer is depthwise or not')
flags.DEFINE_float('dropout', 0.1, 'Dropout rate (1 - keep probability).  0.15 maybe?')
...
A0=[];A1=[];A2=[]
ND=np.zeros((len(A),1)) 
FF=np.zeros((len(A),nmax,3+1)) #3  21
YY=np.zeros((len(A),6))
...
for i in range(0,len(A)):  
    ...
    if kerneltype=='cheb':
        ...
        a0=U[i].dot(np.diag( v-1 ).dot(U[i].T))        
        SP[i,1,0:n,0:n]=a0
        a1=U[i].dot(np.diag( 2*v*v-4*v+1 ).dot(U[i].T))        
        SP[i,2,0:n,0:n]=a1
...
for iter in range(0,20):
    ...
    fname='logs/enzyms_label_'+ kerneltype+'_nk'+str(nkernel)+'_'+str(iter)+'.csv'
