...
flags.DEFINE_list('hidden', [200,200,'meanmax',-100,-2], 'Number of units in each layer negative:denselayer, positive:ConvGraph layer ')
...
flags.DEFINE_list('biases', [False,False,None,True,True,True], 'if apply bias on layers')
flags.DEFINE_list('isdroput_inp', [False,False,None,False,False,False], 'if apply dropout on layers'' input')
flags.DEFINE_list('isdroput_kernel', [False,False,None,None,None], 'if apply dropout on layers'' kernel')
...
flags.DEFINE_float('weight_decay_depthwise', 0, 'Weight for L2 loss on depthwise weigths.')
flags.DEFINE_integer('nkernel', 2,'number of kernels')  
...
a=sio.loadmat('data/protein.mat')
...
A0=[];A1=[];A2=[]
ND=np.zeros((len(A),1)) 
FF=np.zeros((len(A),nmax,3+1)) 
YY=np.zeros((len(A),2))
SP=np.zeros((len(A),nkernel,nmax,nmax))
AA=np.zeros((len(A),nmax,nmax))
...
for i in range(0,len(A)):  
    n=F[i].shape[0]   
    # add node label codes into feature vector 
    FF[i,0:n,0:3]= F[i][:,0:3]  
    # add node degree into feature vector
    ...
    if kerneltype=='cheb':
        ...
        SP[i,0,0:n,0:n]=U[i].dot(np.diag( 1-(v/2)**1 ).dot(U[i].T)) 
        SP[i,1,0:n,0:n]=U[i].dot(np.diag( v/2 ).dot(U[i].T)) 
...
for iter in range(0,20):
    ...
    fname='logs/protein_label_'+ kerneltype+'_nk'+str(nkernel)+'_'+str(iter)+'.csv'
